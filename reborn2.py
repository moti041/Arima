...
def t_delay_snell( ys_v,vy):
    df = pd.DataFrame(columns=["x", "tau1", 'teta1', 'ys'])
    dy = np.diff(ys_v)[1]
    for teta1 in np.arange(0.001,-0.01+pi/2,(pi/2)/45):
        for n in range(len(ys_v)-1): # y val of reflect
            x, tau1, teta_n = 0,0 ,teta1
            for i in range(n-1):
               v1,v2=vy[i,0], vy[i+1,0]
...
for xr in xr_v:
    ...
    # Evaluate the Ricker wavelet at t0 = t - t_delay
    # Initialize an array to store the Ricker wavelet values
    wavelet_values = np.zeros((len(xs), len(ys), len(t) * len(xr_v)))
    wavelet_sctter = np.zeros((len(xs), len(ys), len(t) * len(xr_v)))
    re = np.zeros(( len(xs),len(ys), )) # reflection coeff
    re_scater = np.zeros((len(xs), len(ys),))  # reflection coeff only for scatter
    # Central frequency of the Ricker wavelet
    tr=1 # transmit
    for j in range(len(ys)-1):
        tr *= 4 * vy[ j ,0] * vy[ j + 1,0]/ ((vy[ j,0] + vy[ j + 1,0])**2) # multiplication of tx_coef at 2 ways
        for i in range(len(xs)):
            specular_xs =(xr+xt)/2 # due to specular reflection
            absolute_differences = np.abs(xs_v - specular_xs)
            idx = np.argmin(absolute_differences)
            mask = (xs[i, j] >= xs_interval[0]) & (xs[i, j] <= xs_interval[1]) & (ys[i, j] >= ys_interval[0]) & (
                        ys[i, j] <= ys_interval[1])

            tr_mask =  tr
            re[i,j ]  = np.abs((vy[j + 1, 0] - vy[j, 0]) / (vy[j, 0] + vy[j + 1, 0])) # reflection coeff
            re_scater[i, j] = -1 if mask else 0 # reflection coeff
            t0 = t - 2*find_tau(propagate_vars,xs[0,i],ys[j,0]) # multiply by 2 due to specular reflection
            # t0 -= np.mean(t0)
            wavelet_values[i,j, rx_idx*len(t):(rx_idx+1)*len(t)] = (tr_mask ) *  ricker_wavelet(t0, f) if i==idx else 0 #
            # to for specular
            t0 = t -  find_tau(propagate_vars, xs[0, i], ys[j, 0]) - find_tau(propagate_vars, np.abs(xr-xs[0, i]), ys[j, 0])  # multiply by 2 due to specular reflection
            wavelet_sctter[i, j, rx_idx * len(t):(rx_idx + 1) * len(t)] = (tr_mask) * ricker_wavelet(t0,f)   #
            # for i in range(len(xs)):
            #     mask = (xs[i, j] >= xs_interval[0]) & (xs[i, j] <= xs_interval[1]) & (ys[i, j] >= ys_interval[0]) & (ys[i, j] <= ys_interval[1])
            #     tr_mask = 0 if mask else tr
            #     re = np.abs((vy[ j+1,i] - vy[ j,i])/ (vy[ j,i] + vy[ j + 1,i]))
            ...
            #     wavelet_values[j, :] = (tr_mask**1) * re * ricker_wavelet(t0, f) #
    # Sum wavelet values over xs and ys for each t value
wavelet_values_reshape = wavelet_values.reshape(wavelet_values.shape[0]*wavelet_values.shape[1],wavelet_values.shape[2])
wavelet_sctter_reshape = wavelet_sctter.reshape(wavelet_sctter.shape[0]*wavelet_sctter.shape[1],wavelet_sctter.shape[2])
re_reshape = re.reshape(re.shape[0]*re.shape[1])
re_scater_reshape = re_scater.reshape(re_scater.shape[0]*re_scater.shape[1])
recived_signal = wavelet_values_reshape.T@ re_reshape + wavelet_sctter_reshape.T@ re_scater_reshape
summed_wavelet = np.sum(wavelet_values, axis=(0))
# Plotting a single Ricker wavelet for visualization
    plt.plot(t,summed_wavelet, label=xr)
plt.legend()
